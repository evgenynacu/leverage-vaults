...
| 2 | FlashLoanRouter base: executeFlashLoan, transient storage (initiator + active flag), callback validation, zero-fee enforcement, onFlashLoan callback to initiator | FlashLoanRouter | 1 | d:flash-callback, d:flr-invariants, d:flr-state, d:flr-access |
...
| 7 | Vault core: ERC-20 shares, FIFO deposit/redeem queues with partial fills, requestDeposit (returns requestId), cancelDeposit, requestRedeem (returns requestId), cancelRedeem, minimum amounts, pause/unpause | Vault | 1 | d:accounting, d:cancel, d:partial-epoch, d:epoch-separation, d:min-amounts, d:pause-scope, d:naming |
| 8 | Vault epoch processing: processDeposits (delta NAV mint), processRedeems (fraction-based unwind, burn, distribute), _forceAccrue via totalAssets, reentrancy lock (EIP-1153), FlashLoanRouter parameter validation against Factory registry | Vault | 3, 7 | d:nav-snap, d:epoch-separation, d:partial-epoch, d:reentrancy-lock, d:accrue-before-snap, d:rounding, d:flr-selection |
...
| 10 | Vault migration interface: depositCustom (arithmetic NAV validation, mint shares, returns sharesMinted), redeemCustom (fraction, burn shares, no pending redeem check, returns collateralOut) | Vault | 3, 7, 8 | d:migration-deposit, d:dc-signature, d:dc-access, d:dc-oracle, d:arith-nav, d:redeem-custom-flow |
| 11 | Vault admin: setTolerance, setMigrationRouter, setMinDeposit, setMinRedeem, setGuardian, setKeeper, Ownable2Step (renounce disabled), totalAssets view | Vault | 7 | d:tolerance-params, d:access-details, d:admin-transfer |
...
| 13 | Factory: deploy (beacon proxy creation, vault-strategy linking, on-chain validation), setMigrationRouter, registerRouter/deregisterRouter/isRegisteredRouter, registry | Factory | 7, 3, 2 | d:factory-validation, d:new-protocol, d:new-flashloan, d:vault-beacon, d:beacon-owner, d:flr-selection |
...
- **Vault**: requestDeposit, cancelDeposit, requestRedeem, cancelRedeem (task 7); processDeposits, processRedeems (task 8); syncRedeem (task 9); depositCustom, redeemCustom (task 10); pause, unpause, setTolerance, setMigrationRouter, setMinDeposit, setMinRedeem, setGuardian, setKeeper, totalAssets (task 11)
...
- **FlashLoanRouter**: executeFlashLoan (task 2); per-provider callbacks (task 14)
- **IFlashLoanReceiver**: onFlashLoan — implemented by Strategy (task 3) and MigrationRouter (task 12)
- **MigrationRouter**: migrate, onFlashLoan (task 12)
- **Factory**: deploy, setMigrationRouter, registerRouter, deregisterRouter, isRegisteredRouter (task 13)
